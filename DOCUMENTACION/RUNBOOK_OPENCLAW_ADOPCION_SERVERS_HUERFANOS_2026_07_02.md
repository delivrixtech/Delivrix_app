...
4. **Verificación post-adopción (obligatoria, read-only)**: volver a invocar `read_infrastructure_inventory` (autoritativa) y confirmar que `slug`, `ipv4` y `accountId` coinciden con lo adoptado. Si no coinciden, NO usar `configure_complete_smtp`; abrir rollback manual auditado y corregir el inventario antes de cualquier paso con costo.
