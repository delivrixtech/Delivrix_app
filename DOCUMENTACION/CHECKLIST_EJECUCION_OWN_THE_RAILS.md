...
- [ ] **W4 [S]** (Dev) Wire-up placement-check → scheduler (cerrar el lazo). **DoD:** las dos rutas quedan realimentadas. [Seam HECHO + testeado (sin commitear): el scheduler (`warmup-ramp.ts`) ahora consume señales externas vía dep opcional `getWarmupSignals`, corre el breaker tras cada batch y auto-pausa con `auto_spam_rate`/`auto_placement` (no solo bounce). Sin la dep, comportamiento idéntico (cero regresión). +2 tests de integración verdes; suite completa 1301 pass. Falta el FEED de producción: que `main.ts` pase `getWarmupSignals` leyendo `placement-check`/FBL reales.]
