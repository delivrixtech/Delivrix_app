...
- [ ] **O3 [S]** (Dev) Captura de bounces/DSN sin SSH del agente: colector que lee `mail.log` por SSH (gateway) → parser → DB → expone en `read_send_results`. **DoD:** el agente ve el motivo de rebote por mensaje. [parser hecho + testeado; falta store + colector + exponer]
