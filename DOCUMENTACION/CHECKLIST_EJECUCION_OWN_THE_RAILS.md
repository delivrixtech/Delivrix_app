...
- [ ] **O4 [S]** (Dev) Endurecer diagnóstico de OpenClaw (inbound vs outbound 25, DKIM/selectores, no marcar rojo lo que funciona). **DoD:** no repite las falsas alarmas auditadas. [En curso: root-cause hallado — el único chequeo de :25 era inbound (`ss -tlnp`); faltaba outbound. Motor `openclaw-smtp-reachability.ts` hecho + 9/9 verde: separa inbound/outbound y devuelve `unknown` (nunca un `blocked` falso). Pendiente: wiring read-tool (después de que O3 mergee) + pieza DKIM/selector. SIN commitear; archivos nuevos aislados, no chocan con O3.]
