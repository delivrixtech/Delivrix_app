...
load_admin_proxy_env() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    key="${key%"${key##*[![:space:]]}"}"
    case "${key}" in
      ADMIN_PANEL_GATEWAY_ORIGIN|CANVAS_LIVE_STREAM_TOKEN|GATEWAY_LOG_STREAM_TOKEN|DELIVRIX_READ_BOUNDARY_TOKEN|DELIVRIX_OPENCLAW_TOKEN|OPENCLAW_GATEWAY_TOKEN|WARMUP_API_KEY|VITE_CANVAS_LIVE_STREAM_TOKEN|VITE_GATEWAY_LOG_STREAM_TOKEN|VITE_DELIVRIX_READ_BOUNDARY_TOKEN|VITE_DELIVRIX_OPENCLAW_TOKEN)
        if [[ "${value}" == \"*\" && "${value}" == *\" ]]; then
